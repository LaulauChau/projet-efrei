  ...
  if (!empty($_POST['email']) && !empty($_POST['password'])) {
    ...
    while ($email_verification = $req->fetch()) {
      if ($email_verification['numberEmail'] != 1) {
        header('location: connexion.php?error=1&message=Cette adresse email est déjà utilisée.');
        exit();
      }
    }
    // CONNEXION
    ...
    while ($user = $req->fetch()) {
      if ($password == $user['password'] && $user['blocked'] == 0) {
        ...
        header('location: connexion.php?error=1&Impossible-de-vous-authentifier-correctement.');
        exit();
      }
    }
  }
...
?>
			...
			<form method="post" action="connexion.php">
				...
				<!-- <a href="../espace_client/recuperation.php" title="J'ai oublié mon mot de passe et je souhaite en mettre un nouveau">Mot de passe oublié</a></label>
				<br/><br/> -->
	...
	<!-- <?php
include("../src/boutiqueFooter.php");
?> -->
